...
df = pd.read_excel(os.path.join(folder_path,'final_output.xlsx'))
...
for x in tqdm(df['Text'],leave=True,position=0):
    try:
        country=x
        prompt="Which of the following topics (Price,Quality,Design,Customer Service, Store) are being mentioned. Extract the sub-sentence for each topic. Answer by stating the topic: subsentence. {country}"
...
df_test_3['Topic'] = df_test_3['Topic_Split_3'].str.lower().str.extract('(fit|quality|design|price|size|color|comfort)')[0].str.title().replace({'Size':'Fit',
                                                                                                                                         'Color':'Design',
                                                                                                                                         'Comfort':'Fit'}).fillna("Not Available")
output = df.copy().merge(df_test_3,
                                   on='Review ID')
...
output.to_excel(os.path.join(folder_path,"gpt_output_.xlsx"))